...
<div id="crud-players-form-container" class="form-crud-container container" target-controller="players"> @if($update?? ""==true)
    <form id="players-form" class=" form-crud" method="POST" action="{{ route('players.update',['id'=>$player->id]?? "") }}" enctype="multipart/form-data">
         @csrf
         @method('PUT')
    @else
   <form  id="players-form" class=" form-crud" method="POST" action="{{ route('players.store') }}" enctype="multipart/form-data">
    @endif
...
<div class="form-group">                    
    <label for="player_avatar">Zdjecie</label>
    <input type="file" name="avatar" class="form-control " id="player_avatar" aria-describedby="avatar" placeholder="Zdjecie" value="{{ old('avatar',$player->avatar ?? "") }}">
    <small id="player_avatar_error" class="form-text text-muted"></small>
</div>
